...
public class KeyEditor extends LinearLayout implements Editor, OnClickListener {
    ...
    protected void onFinishInflate() {
        ...
        Choice choices[] = {
                new Choice(Id.pilihan.usage.sign_only, getResources().getString(
                        R.string.choice_sign_only)),
                new Choice(Id.pilihan.usage.encrypt_only, getResources().getString(
                        R.string.choice_encrypt_only)),
                new Choice(Id.pilihan.usage.sign_and_encrypt, getResources().getString(
        ...
        mExpiryDateButton.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                ...
                dialog.setButton(Dialog.BUTTON_NEGATIVE, getContext()
                        .getString(R.string.tmbl_no_date), new DialogInterface.OnClickListener() {
    ...
    public void setValue(PGPSecretKey key, boolean isMasterKey, int usage) {
        ...
        if (!isElGamalKey) {
            choices.add(new Choice(Id.pilihan.usage.sign_only, getResources().getString(
                    R.string.choice_sign_only)));
        }
        if (!mIsMasterKey && !isDSAKey) {
            choices.add(new Choice(Id.pilihan.usage.encrypt_only, getResources().getString(
                    R.string.choice_encrypt_only)));
        }
        if (!isElGamalKey && !isDSAKey) {
            choices.add(new Choice(Id.pilihan.usage.sign_and_encrypt, getResources().getString(
                    R.string.choice_sign_and_encrypt)));
        }
        ...
        if (PgpKeyHelper.isEncryptionKey(key)) {
            if (PgpKeyHelper.isSigningKey(key)) {
                selectId = Id.pilihan.usage.sign_and_encrypt;
            } else {
                selectId = Id.pilihan.usage.encrypt_only;
            }
        } else {
            ...
            if (usage != -1) {
                ...
                selectId = Id.pilihan.usage.sign_only;
            }
