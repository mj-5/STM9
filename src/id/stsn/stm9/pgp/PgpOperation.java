...
public class PgpOperation {
	...
	public Bundle decryptAndVerify(String passphrase) throws IOException,
	PgpGeneralException, PGPException, SignatureException {
		...
		updateProgress(R.string.proses_reading_data, currentProgress, 100);
		...
		//    		}
		//
		//    		updateProgress(R.string.proses_preparing_streams, currentProgress, 100);
		//
		...
		//    		currentProgress += 5;
		//    	} else {
		updateProgress(R.string.proses_finding_key, currentProgress, 100);
		...
		currentProgress += 5;
		updateProgress(R.string.proses_extracting_key, currentProgress, 100);
		...
		if (dataChunk instanceof PGPCompressedData) {
			updateProgress(R.string.proses_decompressing_data, currentProgress, 100);
		...
		if (dataChunk instanceof PGPOnePassSignatureList) {
			updateProgress(R.string.proses_processing_signature, currentProgress, 100);

			returnData.putBoolean(KeyIntentService.RESULT_SIGNATURE, true);
			...
			for (int i = 0; i < sigList.size(); ++i) {
				...
				if (signatureKey == null) {
					...
					returnData.putString(KeyIntentService.RESULT_SIGNATURE_USER_ID, userId);
					break;
				}
			}

			returnData.putLong(KeyIntentService.RESULT_SIGNATURE_KEY_ID, signatureKeyId);
			...
			if (signature != null) {
				...
				returnData.putBoolean(KeyIntentService.RESULT_SIGNATURE_UNKNOWN, true);
			}
		...
		if (dataChunk instanceof PGPLiteralData) {
			updateProgress(R.string.proses_decrypting, currentProgress, 100);
			...
			while ((n = dataIn.read(buffer)) > 0) {
				...
				if (signature != null) {
					try {
						...
						returnData
						.putBoolean(KeyIntentService.RESULT_SIGNATURE_SUCCESS, false);
						signature = null;
					}
				}
			...
			if (signature != null) {
				updateProgress(R.string.proses_verifying_signature, 90, 100);

				PGPSignatureList signatureList = (PGPSignatureList) plainFact.nextObject();
				PGPSignature messageSignature = signatureList.get(signatureIndex);
				if (signature.verify(messageSignature)) {
					returnData.putBoolean(KeyIntentService.RESULT_SIGNATURE_SUCCESS, true);
				} else {
					returnData.putBoolean(KeyIntentService.RESULT_SIGNATURE_SUCCESS, false);
				}
			}
		}
		...
		if (encryptedData.isIntegrityProtected()) {
			updateProgress(R.string.proses_verifying_integrity, 95, 100);
...
//	public Bundle verifyText(boolean lookupUnknownKey) throws IOException, PgpGeneralException,
	...
	PGPException, SignatureException {
		...
		updateProgress(R.string.proses_done, 0, 100);
		...
		updateProgress(R.string.proses_processing_signature, 60, 100);
...
//				returnData.putLong(KeyIntentService.RESULT_SIGNATURE_KEY_ID, signatureKeyId);
//				returnData.putBoolean(KeyIntentService.RESULT_SIGNATURE_LOOKUP_KEY, true);
//
...
//			}
		...
		returnData.putBoolean(KeyIntentService.RESULT_SIGNATURE_SUCCESS, signature.verify());
	...
	/**
	 * can delete it - testing decrypt with symmetric
	 */
